  ...
  return (
    <div className={cn("pointer-events-none fixed inset-0 z-[80]", open && "pointer-events-auto")}>
      <button
        ...
        onClick={onClose}
        className={cn(
          "absolute inset-0 bg-scrim-soft opacity-0 transition-opacity",
          open && "opacity-100"
        )}
      />
      ...
      <aside
        className={cn(
          "absolute left-0 top-0 flex h-full w-full max-w-md flex-col border-r border-text/8 bg-background shadow-[16px_0_40px_var(--shadow-color-drawer)] transition-transform duration-300",
          open ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div className="flex items-center justify-between border-b border-text/8 px-4 py-4 sm:px-5">
          <div>
          ...
          <Button
            ...
            variant="ghost"
            onClick={onClose}
            className="rounded-full text-text/70 hover:bg-surface-soft hover:text-text"
        ...
        <div ref={messageListRef} className="flex-1 space-y-3 overflow-y-auto bg-surface-panel px-4 py-4 sm:px-5">
          {messages.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-text/10 bg-background px-4 py-8 text-center">
        ...
        <form onSubmit={handleSubmit} className="border-t border-text/8 bg-background px-4 py-4 sm:px-5">
          <div className="flex items-end gap-3">
            <label className="min-w-0 flex-1">
              ...
              <input
                ...
                className="h-11 w-full rounded-2xl bg-surface-muted px-4 text-sm text-text ring-1 ring-text/6 outline-none transition focus-visible:ring-2 focus-visible:ring-text/15"
              />
            </label>
  ...
  function renderMessage(message: EchoChatMessage) {
    return (
      <div
        key={message.id}
        className={cn(
          "max-w-[88%] rounded-2xl px-4 py-3 text-sm shadow-[0_4px_12px_var(--shadow-color-soft)]",
          message.kind === "sent" && "ml-auto bg-dark-button text-white",
          message.kind === "received" && "bg-background text-text ring-1 ring-text/6",
          message.kind === "system" && "mx-auto bg-surface-muted text-center text-text/60 ring-1 ring-text/6"
        )}
      >
        <div>{message.text}</div>
        <div className={cn("mt-1 text-[11px]", message.kind === "sent" ? "text-background/70" : "text-text/40")}>
